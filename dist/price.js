...
const PriceTracker=(()=>{
 ...
 function roster(){return [...new Map([...(payload?.catalog||[]),...(payload?.rows||[])].filter(r=>r.molecule===el('aspMolecule').value).map(r=>[r.brand,r])).values()].sort((a,b)=>(a.kind==='reference'?-1:b.kind==='reference'?1:0)||a.brand.localeCompare(b.brand));}
 function brandColor(brand){const ps=roster();return colors[Math.max(0,ps.findIndex(r=>r.brand===brand))%colors.length];}
 ...
 function exportCsv(allProducts=false){const rows=allProducts?(payload?.rows||[]):current(),unit=el('aspUnit').value;if(!rows.length)return;const headers=['payment_quarter','molecule','brand','hcpcs','unit','payment_limit_usd','estimated_asp_usd','addon_pct','method','notes','source_url','source_checked_at','publication_status'];const lines=rows.map(r=>[r.quarter,r.molecule,r.brand,r.hcpcs,unit==='standard'?r.standardDose:r.billingUnit,value(r,'paymentLimit',unit),value(r,'asp',unit),r.addonPct,r.method,r.notes,r.sourceUrl,r.sourceCheckedAt,r.publicationStatus||'Archived snapshot']);const blob=new Blob(['\ufeff'+[headers,...lines].map(line=>line.map(csvCell).join(',')).join('\r\n')],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='CMS-ASP-'+(allProducts?'All-products-all-quarters':el('aspMolecule').value)+'.csv';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
 async function load(){
  if(loading)return;loading=true;el('aspRefresh').disabled=true;el('aspExportAll').disabled=true;el('aspStatus').textContent='게시본을 불러오는 중입니다.';
  try{const r=await fetch('asp.json',{cache:'no-store'});if(!r.ok)throw Error('ASP 게시본을 불러오지 못했습니다.');const next=await r.json();if(next.schemaVersion!==1||!Array.isArray(next.rows)||!next.rows.length)throw Error('ASP 데이터 형식을 확인해 주세요.');payload=next;window.dispatchEvent(new CustomEvent('cms-asp-updated',{detail:payload}));
   const old=el('aspMolecule').value,molecules=[...new Set([...(payload.catalog||[]),...payload.rows].map(r=>r.molecule))].sort();el('aspMolecule').innerHTML=options(molecules,molecules.includes(old)?old:molecules.includes('Trastuzumab')?'Trastuzumab':molecules[0]);el('aspMolecule').disabled=false;
   el('aspFreshness').textContent=payload.cmsCheckedAt?'CMS 확인 '+date(payload.cmsCheckedAt)+' · '+payload.rows.length+' records · '+(payload.catalog||[]).length+' approved brands':'기존 저장본 · 저장소 수정 '+date(payload.sourceRepositoryUpdatedAt)+' · CMS 재확인 전';
   el('aspCoverage').innerHTML='<p>'+esc(payload.coverage)+'</p><p>분기 범위 '+esc([...new Set(payload.rows.map(r=>r.quarter))].sort().join(', '))+'</p>'+(payload.collectionErrors?.length?'<p>일부 소스에 수집 제한이 있습니다. Crosswalk 오류 시 제품명·suffix 매칭은 유지하며, 가격 파일 오류 시 이전 게시값을 유지합니다.</p><ul>'+payload.collectionErrors.map(x=>'<li>'+esc(x.quarter)+': '+esc(x.error)+'</li>').join('')+'</ul>':'')+'<p><a href="'+esc(payload.sourceRepository)+'" target="_blank" rel="noopener">기존 ASP 대시보드 로직 출처 ↗</a></p>';selectMolecule();
