...
const PriceTracker=(()=>{
 ...
 function selectMolecule(){
  ...
  el('aspProducts').innerHTML=products.map((r,i)=>'<label><input type="checkbox" value="'+esc(r.brand)+'" checked><span style="color:'+colors[i%colors.length]+'">'+esc(r.brand)+(r.kind==='reference'?' · Originator':'')+'</span></label>').join('');render();
 }
