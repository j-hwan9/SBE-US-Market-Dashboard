...
function method(){
 showDialog('데이터 기준과 업데이트',`<p><strong>Purple Book 월말 기준 ${esc(formatDate(DATA.purpleBookAsOf||DATA.purpleBookDate))}</strong><br>소스 확인 ${esc(formatDate(DATA.retrievedAt))} · ${DATA.products.length}개 BLA·제품 항목</p><p><a href="${esc(DATA.purpleBookUrl)}" target="_blank" rel="noopener">공식 Purple Book CSV</a></p><ul><li>그래프는 성분별 고유 브랜드 수입니다. 오리지네이터를 제외하며 같은 브랜드의 여러 BLA·presentation은 한 번만 셉니다. Interchangeable 제품도 포함합니다. 판매 여부와는 별개입니다.</li><li>비교표는 오리지네이터를 맨 왼쪽에 고정합니다. Denosumab처럼 reference product가 둘이면 Originator 선택으로 구분합니다. 같은 오리지네이터의 여러 BLA는 한 열에 합치고 원문은 각각 제공합니다.</li><li>수동 업데이트는 GitHub의 Daily regulatory and ASP refresh → Run workflow에서 실행합니다. 자동 확인은 매일 06:00 KST에 시작합니다. 변경이 없으면 확인 시각만 갱신하고, 실제 변경이 있을 때만 전체 데이터와 변경 내역을 이력으로 보관합니다.</li><li>새 CSV·PI를 확인하고 검증에 통과한 경우만 게시 데이터를 교체합니다. 오류가 발생하면 기존 게시본을 유지하고 실행 기록에 원인을 남깁니다.</li><li>조회 시점에서 과거 저장본을 열 수 있습니다. 변경 이력은 신규·제외 제품과 변경된 항목을 표시합니다. 저장을 시작하기 이전 시점의 PI는 복원한 자료가 아닙니다.</li><li>수치·날짜의 UI 표시는 MMM DD, YYYY입니다. 원문 PI 인용문은 출처 문구를 보존합니다. Purple Book은 월간 자료이므로 표시일은 해당 월말 기준일입니다.</li><li>Interchangeability는 특정 reference product·presentation에 대한 지정입니다. 다른 biosimilar끼리의 대체 가능성을 의미하지 않습니다.</li><li>PI에 없는 shelf life, sorbitol-free 또는 latex-free를 추정하지 않습니다. NDC는 해당 라벨의 포장 정보이며 현재 구매 가능 여부를 보증하지 않습니다.</li><li>문구 차이는 임상적 동등성이나 우열의 판정이 아닙니다. 조제·희석 후 조건과 미개봉 보관조건, 제품별 투여경로를 구분해서 확인하세요.</li></ul>`);
}
...
function refreshDialog(){
 ...
 if(!/^[\w.-]+\/[\w.-]+$/.test(repo||''))return showDialog('업데이트 연결 준비',`<p>공식 소스를 다시 수집하는 코드와 주별 실행 설정은 준비되어 있습니다. GitHub 저장소를 연결하면 실행할 수 있습니다.</p><p>연결 후 이 버튼에서 <strong>Daily regulatory and ASP refresh → Run workflow</strong>를 열 수 있습니다. 현재 버튼은 소스 수집을 실행하지 않습니다.</p>`);
