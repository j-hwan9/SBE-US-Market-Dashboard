...
First captures establish **Baseline**. Subsequent captures compare normalized text hashes with short excerpts, links/resources, image URLs, PDF file hashes (up to 6 linked PDFs per page, 8 MB per PDF), and a fixed-viewport screenshot (full page). Added/removed text excerpts are capped; counts reflect all captured changed lines. Pixel differences are **Visual review** candidates, not AI claims of commercial significance. Footer copyright/cookie noise is excluded. JavaScript content is rendered using Playwright. Login, CAPTCHA, robots exclusions and errors are reported as **Needs review**, never as unchanged or deleted. Successful old evidence is retained after failures.
