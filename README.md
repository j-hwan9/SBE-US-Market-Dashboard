...
Home Price snapshot reads the same `dist/asp.json` as Price tracker. WAC remains `-`. ASP matches the selected portfolio brand (including explicit slash-separated shared-HCPCS names) and molecule, and shows the latest valid **standardized-dose estimated ASP**, its payment quarter, source and preliminary status. No matching calculated ASP displays `N/A`; load failure is reported separately. If a newer record cannot yield ASP, the latest valid older quarter is explicitly identified. Multiple HCPCS values remain separate. Refreshing the ASP publication in Price tracker also updates Home.
