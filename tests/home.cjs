...
assert.equal(run("PortfolioHome.previousQuarter('2026 Q1')"),'2025 Q4');
context.changeRows=[{molecule:'Denosumab',brand:'Ospomyv',reference:'Prolia',hcpcs:'Q9991',quarter:'2026 Q2',estimatedAsp:10,standardFactor:120,standardDose:'120 mg'},{molecule:'Denosumab',brand:'Ospomyv',reference:'Prolia',hcpcs:'Q9991',quarter:'2026 Q3',estimatedAsp:8,standardFactor:120,standardDose:'120 mg'}];
assert(Math.abs(run('PortfolioHome.aspChange(changeRows,changeRows[1])')+20)<1e-9);
assert.equal(run('PortfolioHome.aspChange([changeRows[1]],changeRows[1])'),null);
context.changeRows[0].quarter='2026 Q1';assert.equal(run('PortfolioHome.aspChange(changeRows,changeRows[1])'),null);
context.changeRows[0].quarter='2026 Q2';context.changeRows[0].hcpcs='Q9992';assert.equal(run('PortfolioHome.aspChange(changeRows,changeRows[1])'),null);
console.log('Home QoQ: adjacent quarter, same HCPCS, year boundary and missing data verified.');
